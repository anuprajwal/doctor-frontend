    ...
    <CallProvider>
      <div className="min-h-screen flex flex-col bg-slate-50 print:bg-white">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 print:p-0">
          ...
          {/* Render directly without gating on parent state so component can hydrate from storage */}
          {currentTab === 'appointment-detail' && (
            <AppointmentDetails 
              appointment={selectedAppointment || getSelectedItem('doctor_appointment')} 
              onBack={() => {
                clearSelectedItem('doctor_appointment');
          ...
          {/* Render directly without gating on parent state */}
          {currentTab === 'hospital-detail' && (
            <HospitalDetailPage 
              hospital={selectedHospital || getSelectedItem('doctor_hospital')} 
              onBack={() => {
                clearSelectedItem('doctor_hospital');
