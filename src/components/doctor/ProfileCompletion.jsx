...
//       // Ensure state is explicitly set as an array to prevent .map errors
...
//       setStatus({ loading: false, error: null, success: 'Profile photo successfully removed.' });
//     } catch (err) {
...
//           <p className="text-xs text-slate-400 mt-1">Manage core institutional variables and profile attributes.</p>
//         </div>
...
//           <InputField label="Beneficiary Name" value={bankData.beneficiary_name} onChange={(e) => setBankData({ ...bankData, beneficiary_name: e.target.value })} placeholder="e.g. RAJU BANK" />
...
//             <button type="submit" className="px-4 py-2 bg-slate-800 text-white text-xs font-bold rounded-lg hover:bg-slate-900 transition">Save Settlement Account</button>
//           </div>
//         </form>
//       </div>
...
//             {editingAddressId && <button type="button" onClick={() => { setEditingAddressId(null); setAddressForm({ city: '', pincode: '', street: '', state: '', country: '', landmark: '', houseNo: '' }); }} className="px-3 py-1.5 border rounded-lg text-xs font-semibold">Cancel</button>}
//             <button type="submit" className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition">{editingAddressId ? 'Sync Updates' : 'Inject Address Node'}</button>
//           </div>
//         </form>
//       </div>
//     </div>
...
export default function ProfileCompletion() {
  ...
  const fetchAddressesData = async () => {
    try {
      const res = await doctorService.getAllAddresses();
      // Ensure state is explicitly an array to prevent .map errors
      const dataList = Array.isArray(res.data) ? res.data : (res.data?.addresses || []);
      setAddresses(dataList);
    } catch (err) {
      setAddresses([]);
  ...
  return (
    <div className="max-w-4xl mx-auto my-10 space-y-8">
      ...
      {/* 2. Embedded Razorpay Banking KYC Verification */}
      <DoctorKycSection doctorId={formData.id || '1'} />
