...
export default function PrescriptionWorkspace({ appointment, onBack }) {
  const [medications, setMedications] = useState([
    { id: 1, drug: 'pcm', qty: '1', timing: 'morning, afternoon, evening', notes: 'take after meals' }
  ]);
    ...
    try {
      ...
      if (fetchedMeds.length > 0) {
        setMedications(fetchedMeds.map((m, idx) => ({
          id: m.id || idx + 1,
          drug: m.drug || m.name || '',
          qty: m.qty || m.dosage || '1',
          timing: m.timing || m.frequency || 'morning, evening',
          notes: m.notes || ''
        })));
        setNoPrescriptionState(false);
      } else {
  ...
  const handleAddMedicationRow = () => {
    setMedications(prev => [
      ...prev,
      { id: Date.now(), drug: '', qty: '1', timing: 'morning, evening', notes: '' }
    ]);
    setNoPrescriptionState(false);
  };
  ...
  const executeCommitPrescription = async () => {
    if (medications.length === 0 || medications.some(m => !m.drug.trim())) {
      setStatus({ loading: false, error: 'Please specify drug names for all lines.', success: null });
      return;
    }
    ...
    try {
      ...
      setStatus({ loading: false, error: null, success: 'Prescription saved and appointment closed successfully!' });
      setNoPrescriptionState(false);
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="bg-white border border-slate-200/80 rounded-xl shadow-sm p-6 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div>
            <button onClick={onBack} className="text-xs text-slate-400 font-bold hover:underline mb-1">← Back</button>
            <h2 className="text-base font-bold text-slate-800">Prescription Workspace — Appointment #{appointment.id}</h2>
            <p className="text-xs text-slate-500 mt-0.5">Patient: {appointment.patient?.username || appointment.patientName || 'Patient'}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={onBack} className="px-4 py-2 border border-slate-200 text-slate-600 text-xs font-semibold rounded-lg hover:bg-slate-50 transition">Cancel</button>
            <button onClick={executeCommitPrescription} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg shadow-sm transition">
        ...
        {noPrescriptionState ? (
          <div className="p-8 border border-dashed border-slate-200 rounded-xl bg-slate-50/50 text-center space-y-3">
            ...
            <p className="text-xs text-slate-400 max-w-xs mx-auto">No active medication records mapped to this consultation sequence.</p>
            <button type="button" onClick={handleAddMedicationRow} className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700 transition">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Medications List</h3>
              <button onClick={handleAddMedicationRow} className="text-xs text-blue-600 font-bold hover:underline">+ Add Row</button>
            </div>

            <div className="space-y-4">
              {medications.map((med) => (
                <div key={med.id} className="p-4 border border-slate-200 rounded-xl bg-slate-50/30 grid grid-cols-1 sm:grid-cols-12 gap-3 relative">
                  <div className="sm:col-span-3">
                    ...
                    <input 
                      ...
                      placeholder="e.g. PCM" 
                      className="w-full px-3 py-1.5 border rounded-lg bg-white text-xs outline-none focus:border-blue-500"
                    />
                  </div>
                  ...
                  <div className="sm:col-span-1 flex items-end justify-center pb-1">
                    <button onClick={() => handleRemoveMedicationRow(med.id)} className="text-slate-300 hover:text-rose-600 transition text-base" title="Remove row">🗑️</button>
                  </div>
                </div>
              ))}
            </div>
